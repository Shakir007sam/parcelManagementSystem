...
<title>Officer - Pickup Scheduling</title>
...
<body
	...
	<div id="popup"
		...
		Pickup scheduled successfully.</div>
	<%
	}
	%>
	...
	<div id="popup"
		...
		Error: Invalid Booking ID or date/time.</div>
	<%
	}
	%>
	...
	<header
		...
		<h1 class="text-3xl font-bold text-red-700">Pickup Scheduling</h1>
		<%
	...
	<!-- Scheduling Form -->
	<form action="OfficerPickupSchedulingServlet" method="post"
		...
		<div>
			<label for="bookingId" class="block mb-1 font-semibold text-red-700">Booking
				ID (BK)</label> <input type="text" id="bookingId" name="bookingId" minlength="10"
				maxlength="10" placeholder="Enter last 10-digit of Booking ID" required
				class="w-full px-4 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400" />
			<p class="text-red-500 text-sm mt-1">Booking ID must be exactly
				10 digits.</p>
		</div>

		<!-- Pickup Date -->
		<div>
			<label for="pickupDate" class="block mb-1 font-semibold text-red-700">Pickup
				Date</label> <input type="date" id="pickupDate" name="pickupDate" required
				class="w-full px-4 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400" />
		</div>

		<!-- Pickup Time -->
		<div>
			<label for="pickupTime" class="block mb-1 font-semibold text-red-700">Pickup
				Time</label> <input type="time" id="pickupTime" name="pickupTime" required
				class="w-full px-4 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400" />
		</div>
